...
//             Claim storage thisClaim = theirClaims[i];
//             if (
//                 (0 == bytes(thisClaim.protocol).length) &&
...
//         uint32 _point,
//         address _manager
//     ) external activePointOwner(_point) {
...
//         uint16 _prefix,
//         address _spawnProxy
//     ) external activePointOwner(_prefix) {
...
//         uint8 _galaxy,
//         address _voter
//     ) external activePointOwner(_galaxy) {
...
//             (ecliptic.getSpawnLimit(prefix, now) >
//                 azimuth.getSpawnCount(prefix))
//         ) {
...
//         //
//         if (
//             (!_clean || !azimuth.hasBeenLinked(_point)) &&
...
//     //
//     constructor(Azimuth _azimuth) public TakesPoints(_azimuth) {
//         //
//     }
...
//     //
//     function register(
//         //  _participant: address of the participant
...
//         //
//         require((_rate > 0) && (_rateUnit > 0) && (_amount > 0));
...
//     //
//     function deposit(address _participant, uint16 _star) external onlyOwner {
//         Batch storage batch = batches[_participant];
...
//     //
//     function startReleasing() external onlyOwner {
...
//     //
//     function withdrawOverdue(
//         address _participant,
//         address _to
//     ) external onlyOwner {
//         //  this can only be done :escapeHatchTime after the release start
//         //
//         require((0 < start) && (block.timestamp > start.add(escapeHatchTime)));
...
//         //
//         require(0 != batches[msg.sender].amount && 0 == batches[_to].amount);
...
//         address _participant
//     ) public view returns (uint16 limit) {
...
//                 (block.timestamp.sub(realStart) / batch.rateUnit)
//             );
//         }
...
//         address _participant
//     ) external view returns (uint16[] stars) {
